...
def train_epoch(netG: nn.Module, optimizerG: torch.optim.Optimizer, epoch: int, cuda_batches_queue: Queue,
                generator_criterion: callable, STEPS_PER_EPOCH: int):
...
def main(start_epoch: int, NUM_EPOCHS: int, STEPS_PER_EPOCH: int, batch_size: int, VAL_STEPS: int, val_batch_size: int):
   ...
   #             if k in ['run_name', 'snapshot', 'resume', 'lr']:
    ...
    if start_epoch != 1:
        netG.load_state_dict(torch.load('epochs/cnn/netG_epoch_%d.pth' % (start_epoch - 1)))
    else:
        pass
    ...
    for epoch in range(start_epoch, NUM_EPOCHS+1):
        ...
        torch.save(netG.state_dict(), 'epochs/cnn/netG_epoch_%d.pth' % (epoch))
